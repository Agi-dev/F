 ...
 */
class TechnicalFilesystemInteTest
    extends \F\Technical\Base\Test\Integration\Service
{
    ...
    {
    	$actual = $this->s()->getFileSize($this->getDataSetPath() . '/mire.jpg');
    	$this->assertEquals("170", $actual);
    }
    ...
    {
    	$this->cleanFolderInDataset('filesystem');
    	$p = $this->getDataSetPath();
    	$this->assertInstanceOfService($this->s()->copyFile($p . '/fileExists.txt', $p .'/filesystem/fileCopy.txt'));
    	$this->assertFileExists($p .'/filesystem/fileCopy.txt');
    	$this->cleanFolderInDataset('filesystem');
    }
    ...
    public function testCopyContentDirWithSuccess()
    {
    	$this->cleanFolderInDataset('filesystem');
    	$p = $this->getDataSetPath();
    	$this->assertInstanceOfService($this->s()->copyContentDir($p  . '/dir2copy', $p .'/filesystem'));
    	$this->cleanFolderInDataset('filesystem');
    }
	...
	public function testCleanDirWithSuccess()
	{
		$this->cleanFolderInDataset('filesystem');
		$p = $this->getDataSetPath();
		$this->s()->copyContentDir($p  . '/dir2copy', $p .'/filesystem');
		$this->assertInstanceOfService($this->s()->cleanDir($p .'/filesystem'));
		$this->assertFileNotExists($p .'/filesystem/dir1');
	}
	...
	public function testDeleteDirWithSuccess()
	{
		...
		$p = $this->getDataSetPath();
		$this->s()->copyContentDir($p  . '/dir2copy', $p .'/filesystem');
		$this->assertInstanceOfService($this->s()->DeleteDir($p .'/filesystem/dir1/dir2'));
		$this->assertFileNotExists($p .'/filesystem/dir1/dir2');
	}
}